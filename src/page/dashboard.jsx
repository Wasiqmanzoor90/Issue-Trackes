...
import { Box, Typography, Paper, Button, Chip, Stack, Grid, Skeleton, Avatar } from "@mui/material";
...
function Dashboard() {
  ...
  try {
    const token = getAuthToken(); // Your method to get token from localStorage
    ...
    setProjects(data);
    setDisplayedProjects(data.slice(1, INITIAL_DISPLAY_COUNT));
  ...
  if (authorized === null)
    return (
      <Box sx={{ p: 5, textAlign: "center" }}>
        ...
        <Skeleton variant="rectangular" width="100%" height={220} sx={{ mx: "auto", borderRadius: 3 }} />
      </Box>
    );
  ...
  return (
    <Box sx={{
      ...
      <Stack direction="row" alignItems="center" gap={2} mb={3}>
        <Avatar sx={{ bgcolor: "#6c6fed", width: 48, height: 48, fontWeight: 700, fontSize: 28 }}>
          ...
          <Paper elevation={4} sx={{ p: 3, borderRadius: 3 }}>
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
              <Stack direction="row" alignItems="center" gap={1}>
                ...
                <Typography variant="h6" fontWeight={600}>My Projects</Typography>
              </Stack>
              <Button
                variant="contained"
                startIcon={<AddCircleOutlineIcon />}
                onClick={() => navigate("/project/create")}
                sx={{ bgcolor: "#6c6fed", color: "#fff", fontWeight: 600, fontSize: 14, borderRadius: 2, textTransform: "none" }}
              >
                New
              </Button>
            </Stack>
            <Stack gap={1}>
              {loadingProjects
                ? [1, 2].map(i => <Skeleton key={i} variant="rectangular" height={36} sx={{ borderRadius: 2 }} />)
                : displayedProjects.length > 0 
                  ? displayedProjects.map(project => (
                      <Paper key={project._id} sx={{ p: 1.2, bgcolor: "#f4f6fd", borderRadius: 2, cursor: "pointer", "&:hover": { bgcolor: "#ecefff" } }}>
              ...
              {showMoreProjects && !loadingProjects && (
                <Button
                  ...
                  sx={{ mt: 1, borderColor: "#6c6fed", color: "#6c6fed", textTransform: "none" }}
                >
                  Load More Projects
                </Button>
              )}
          ...
          <Paper elevation={4} sx={{ p: 3, borderRadius: 3 }}>
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
              <Stack direction="row" alignItems="center" gap={1}>
                ...
                <Typography variant="h6" fontWeight={600}>Recent Issues</Typography>
              </Stack>
              <Button
                variant="contained"
                startIcon={<AddCircleOutlineIcon />}
                onClick={() => navigate("/issues/create")}
                sx={{ bgcolor: "#f78c6c", color: "#fff", fontWeight: 600, fontSize: 14, borderRadius: 2, textTransform: "none" }}
              >
                New
              </Button>
            </Stack>
            <Stack gap={1}>
              {loadingIssues
                ...
                : displayedIssues.length > 0
                  ? displayedIssues.map(issue => (
                      <Paper key={issue._id} sx={{ p: 1.2, bgcolor: "#fff5ea", borderRadius: 2, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <Typography fontWeight={500}>{issue.title}</Typography>
                        <Chip
                          label={issue.status === "open" ? "Open" : "Closed"}
                          color={issue.status === "open" ? "success" : "default"}
                          size="small"
                        />
                      </Paper>
                    ))
              ...
              {showMoreIssues && !loadingIssues && (
                <Button
                  ...
                  sx={{ mt: 1, borderColor: "#f78c6c", color: "#f78c6c", textTransform: "none" }}
                >
                  Load More Issues
                </Button>
              )}
        ...
        <Grid item xs={12} md={4}>
          <Paper elevation={4} sx={{ p: 3, borderRadius: 3 }}>
            <Stack direction="row" alignItems="center" gap={1} mb={2}>
              ...
              <Typography variant="h6" fontWeight={600}>Stats</Typography>
            </Stack>
            <Stack gap={1}>
              <Typography fontWeight={500}>Total Projects: {projects.length}</Typography>
              <Typography fontWeight={500}>Open Issues: {issues.filter(i => i.status === "open").length}</Typography>
