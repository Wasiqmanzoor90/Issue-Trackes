...
function AllProject() {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:4000/api/projects", {
      ...
      .catch(err => setError(err.response?.data?.message || "Error fetching projects"))
          ...
          <Stack spacing={3}>
            {projects.map(({ _id, title, description, status }) => (
              ...
              >
                ...
                <Typography color="text.secondary" sx={{ minHeight: 36, fontSize: 16 }}>
                  {description || <span style={{ color: '#bdbdbd' }}>No description provided.</span>}
                </Typography>
              </Paper>
            ))}
          </Stack>
